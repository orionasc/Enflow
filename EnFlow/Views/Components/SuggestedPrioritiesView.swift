...
struct SuggestedPriorityContext {
    ...
    let threePart: EnergyForecastModel.ThreePartEnergy     // normalized 0…1
                    ...
                    ZStack(alignment: .topTrailing) {
                        suggestionCard(for: p)
                            .swipeActions(edge: .trailing, allowsFullSwipe: false) {
                                Button { vm.pin(p) }    label: { Label("Pin",    systemImage: "pin.fill") }
                                Button { vm.snooze(p) } label: { Label("Snooze", systemImage: "clock.fill") }
                                Button { vm.dismiss(p) }label: { Label("Dismiss",systemImage: "xmark.circle") }
                            }
                            .contextMenu {
                                Button("Pin")    { vm.pin(p) }
